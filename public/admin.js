...
function promotions() {
  try {
    ...
    return { amount_gifts: [], quantity_gifts: [], bundle_discounts: [], amount_discounts: [] };
  }
}
  ...
  if (settingsForm && !document.querySelector("#promotionEditor")) {
    settingsForm.insertAdjacentHTML("afterend", `
      <section id="promotionEditor" class="staff-manager promotion-manager">
        ...
        <div class="promo-block">
          <h3>组合套装价</h3>
          <p class="small-muted">勾选一套所需制品并填写套装价。顾客每选齐一组就自动优惠，可重复成套，多买的单品仍按原价计算。</p>
          <div id="bundleDiscountRules" class="promo-list"></div>
          <button class="ghost-btn" type="button" data-add-promo="bundle_discounts">添加组合套装价</button>
        </div>
        <div class="promo-block">
          <h3>满额减价</h3>
...
function promoProductPicker(selectedIds = [], fieldName = "trigger_product_ids") {
  const selected = new Set((selectedIds || []).map(Number));
      ...
      ${products.map((product) => {
        ...
        return `
          <label class="promo-product-option">
            <input type="checkbox" data-promo-product="${fieldName}" value="${product.id}" ${selected.has(product.id) ? "checked" : ""}>
            <span>
...
function promotionRuleMarkup(type, index, rule, fallbackName, body, className = "") {
  const key = `${type}:${index}`;
  const name = rule.name || fallbackName;
  return `
    <details class="promo-row promo-rule ${className}" data-promo-row="${key}" ${adminState.expandedPromotion === key ? "open" : ""}>
      <summary class="promo-rule-summary">
        <strong data-promo-summary-name>${escapeHtml(name)}</strong>
        <span aria-hidden="true"></span>
      </summary>
      <div class="promo-rule-body">
        ${body}
      </div>
    </details>
  `;
...
function renderPromotions() {
  ...
  document.querySelector("#amountGiftRules").innerHTML = data.amount_gifts.map((rule, index) => promotionRuleMarkup(
    "amount_gifts",
    index,
    rule,
    "满额赠品",
  ...
  document.querySelector("#quantityGiftRules").innerHTML = data.quantity_gifts.map((rule, index) => promotionRuleMarkup(
    "quantity_gifts",
    index,
    rule,
    "买件赠品",
    `
      ...
      <div class="promo-field promo-products-field">
        <span>指定商品</span>
        ${promoProductPicker(rule.trigger_product_ids, "trigger_product_ids")}
      </div>
    ...
    `,
    `promo-quantity-rule trigger-${promotionTriggerType(rule)}`,
  )).join("") || `<p class="small-muted">还没有买件赠品规则</p>`;

  document.querySelector("#bundleDiscountRules").innerHTML = data.bundle_discounts.map((rule, index) => promotionRuleMarkup(
    "bundle_discounts",
    index,
    rule,
    "组合套装价",
    `
      <label class="promo-field">规则名
        <input data-promo-field="name" value="${escapeHtml(rule.name || "")}" placeholder="例如 全套钥匙扣">
      </label>
      <label class="promo-field">每套优惠价
        <input data-promo-field="bundle_price" type="number" min="0" step="0.01" value="${rule.bundle_price ?? ""}" placeholder="例如 50">
      </label>
      <div class="promo-field promo-products-field">
        <span>每套包含的制品</span>
        ${promoProductPicker(rule.product_ids, "product_ids")}
      </div>
      <div class="promo-rule-actions">
        <label class="check-row"><input data-promo-field="active" type="checkbox" ${rule.active === false ? "" : "checked"}> 启用</label>
        <button class="ghost-btn" type="button" data-remove-promo="bundle_discounts:${index}">删除</button>
      </div>
    `,
  )).join("") || `<p class="small-muted">还没有组合套装价规则</p>`;

  document.querySelector("#amountDiscountRules").innerHTML = data.amount_discounts.map((rule, index) => promotionRuleMarkup(
    "amount_discounts",
    index,
    rule,
    "满额减价",
...
function collectPromotions() {
  const data = { amount_gifts: [], quantity_gifts: [], bundle_discounts: [], amount_discounts: [] };
  document.querySelectorAll("[data-promo-row]").forEach((row) => {
    ...
    row.querySelectorAll("[data-promo-field]").forEach((field) => {
      ...
      else if (["threshold", "discount", "bundle_price"].includes(key)) rule[key] = Number(field.value || 0);
    ...
    const productFields = new Set(Array.from(row.querySelectorAll("[data-promo-product]")).map((field) => field.dataset.promoProduct));
    productFields.forEach((fieldName) => {
      rule[fieldName] = Array.from(row.querySelectorAll(`[data-promo-product="${fieldName}"]`))
...
function addPromotion(type) {
  ...
  if (type === "bundle_discounts") {
    data.bundle_discounts.push({ name: "组合套装价", product_ids: [], bundle_price: 0, active: true });
  }
  if (type === "amount_discounts") {
    data.amount_discounts.push({ name: "满额减价", threshold: 0, discount: 0, active: true });
  }
  adminState.expandedPromotion = `${type}:${data[type].length - 1}`;
...
document.addEventListener("input", (event) => {
  const promotionEditor = event.target.closest("#promotionEditor");
  if (promotionEditor) {
    adminState.promotionsDirty = true;
    if (event.target.matches('[data-promo-field="name"]')) {
      const summary = event.target.closest("[data-promo-row]")?.querySelector("[data-promo-summary-name]");
      if (summary) summary.textContent = event.target.value.trim() || "未命名规则";
    }
  }
});
document.addEventListener("toggle", (event) => {
  const row = event.target.closest?.("[data-promo-row]");
  if (row) adminState.expandedPromotion = row.open ? row.dataset.promoRow : "";
}, true);
